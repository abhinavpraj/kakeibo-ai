...
def generate_insights(expenses, monthly_income, target_savings):
    ...
    if monthly_income <= 0:
        return [
            "Set your monthly income and savings goal to unlock personalized budget insights."
        ]

    if expenses.empty:
        return [
            "Start by logging today's expenses. Kakeibo works best when spending is reviewed regularly."
        ]
    ...
    if planned_spend and total_spent > planned_spend:
        ...
        insights.append(
            f"You still have {currency(planned_spend - total_spent)} available in this month's planned spending budget."
        )
    ...
    if forecast_savings < target_savings:
        ...
        insights.append(
            f"You are on track to save about {currency(forecast_savings)} this month if spending stays steady."
        )

    category_totals = expenses.groupby("category")["amount"].sum().to_dict()
    discretionary = category_totals.get("Optional (Wants)", 0) + category_totals.get(
        "Extra (Unexpected)", 0
    )
    if total_spent and discretionary / total_spent >= 0.4:
        insights.append(
            "Optional and Extra purchases make up a large share of spending. Add reflections to spot impulse patterns."
        )
